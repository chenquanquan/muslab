...
int main(int argc, char *argv[])
{
    ...
    while (1) {
        ...
        if (note.symbol == 0) {
            freq = get_cpitch_freq(toupper((char)note.pitch), note.level);
        } else if (note.symbol == -1) {
            freq = get_cpitch_flat_freq(toupper((char)note.pitch), note.level);
        } else if (note.symbol == 1) {
            freq = get_cpitch_sharp_freq(toupper((char)note.pitch), note.level);
        }
